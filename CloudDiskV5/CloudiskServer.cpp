...
void CloudiskServer::register_signup_module() {
    ...
    m_server.POST("/user/signup", [](const HttpReq * req, HttpResp * resp, SeriesWork * series) {
        ...
        auto ep = discover_one_healthy("http://127.0.0.1:8500", "UserService");
        if (ep.second <= 0) { /* 兜底：返回服务不可用 */ }
        ::clouddisk::UserService::SRPCClient client(ep.first.c_str(),static_cast<unsigned short>(ep.second));
