...
using std::map;
// 验证token
bool checkAuth(const HttpReq *req,HttpResp *resp, User &user)
{
    HttpHeaderCursor http_cursor{req};
    string key, value;
    string auth_header;
    ...
    if (auth_header.empty())
    {
        ...
        HttpHeaderCursor cookie_cursor{req};
    ...
    if (auth_header.empty() || auth_header.find("Bearer ") != 0)
    {
        ...
        resp->append_output_body("缺少Token");
        return false;
    }
    ...
    {
        resp->set_status_code("401");
        resp->append_output_body("Token 无效或已过期");
        return false;
    }
    return true;
}
